...
awk -v CMPSEED=0 -f ${check_path}/scripts/parse.awk -f ${check_path}/scripts/parse_cplex_callback_ndp.awk -v LINTOL=1e-4 -v MIPGAP=0.01  ${path}/bidirected.cplex_callback_ndp.1threads.7200s.out | tee ${path}/bidirected.cplex_callback_ndp.1threads.7200s.res
rm -f ${path}/bidirected.cplex_callback_ndp.1threads.7200s.out
